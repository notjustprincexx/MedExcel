...
window.addXP = async function(amount) {
    ...
    // Update UI elements
    const uiXP1 = document.getElementById('studyXpDisplay');
    if(uiXP1) uiXP1.textContent = window.formatXP(window.userStats.xp);
...
window.loadQuizOverview = function(id) {
    currentQuiz = window.currentQuiz = window.quizzes.find(q => q.id === id);
...
if (generateBtn) {
    generateBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            currentCardIndex = 0;
            window.currentCardIndex = 0; sessionScore = 0; isMCQMode = window.globalQuizType === "Multiple Choice"; window.isMCQMode = isMCQMode;
            ...
            const deckName = deckNameEl && deckNameEl.value.trim() ? deckNameEl.value.trim() : window.selectedFile.name.split('.')[0].replace(/[-_]/g, ' ');
...
window.selectAvatar = function(index) {
    ...
    if (window.currentUser?.uid) {
        try { updateDoc(doc(db, "users", window.currentUser.uid), { avatarIndex: index }).catch(() => {});
        } catch(e) {}
    }
    window.applyAvatar();
    // Highlight in picker
...
window.initUserUI = async function(user) {
    try {
        ...
        if(uNameEl) uNameEl.textContent = savedName;
        ...
        if(uInitEl) uInitEl.textContent = user.email ? user.email.charAt(0).toUpperCase() : "?";
        ...
        if(qSnapResult) {
            let loadedQuizzes = [];
            qSnapResult.forEach(d => loadedQuizzes.push(d.data()));
        ...
        if (userDoc.exists()) {
            ...
            if (pendingAvatar !== null && pendingAvatar !== '') {
                ...
                try { updateDoc(doc(db, 'users', user.uid), { avatarIndex: parseInt(pendingAvatar) }).catch(()=>{});
                } catch(e) {}
            ...
            if (sessionStorage.getItem('medexcel_just_logged_in')) {
                ...
                const _emailToNotify = user.email || data.email || (localStorage.getItem('nativeUser') ? JSON.parse(localStorage.getItem('nativeUser')||'{}').email : null);
                if (_emailToNotify) {
                    try {
        ...
        if (!hasCheckedInToday) { 
            if(headerDisplay) headerDisplay.textContent = Math.max(0, currentStreakCount - 1); 
            if(headerFireIcon) headerFireIcon.style.opacity = '0.4';
            setTimeout(() => { if (localStorage.getItem('medexcel_onboarding_v1')) { window.openStreakModal(); } else { window._pendingStreakModal = true; } }, 500);
        }
        else { 
            if(headerDisplay) headerDisplay.textContent = currentStreakCount;
            if(headerFireIcon) headerFireIcon.style.opacity = '1'; 
        }

        // Profile & limits UI
        const pStreak = document.getElementById("profileStreakCount");
        if(pStreak) pStreak.textContent = currentStreakCount;
        ...
        const sXp = document.getElementById("studyXpDisplay");
        if(sXp) sXp.textContent = window.formatXP(data.xp || 0);
        if (data.createdAt) {
            let memberDate = null;
...
window.saveNickname = async function() {
    ...
    try {
        ...
        if(uName) uName.textContent = newName;
...
window.showShareGroupCode = async function() {
    ...
    // Web fallback
    if (navigator.share) {
        try { await navigator.share({ title: `Join ${name} on MedExcel`, text: message, url: link });
        } catch(e) {}
        return;
    }
    ...
    setTimeout(() => {
        ...
        area.innerHTML = `
            <div class="card-panel fade-in w-full max-w-2xl">
                ...
                <p class="text-[15px] font-medium text-[var(--text-muted)] mb-8"><i class="fas fa-layer-group mr-1.5"></i>${qLength} ${itemLabel}</p>
                <p class="text-sm text-[var(--text-muted)] mb-8">Shared by ${window.escapeHTML(dk.sharedByName || 'a group member')}</p>
                <button onclick="window.startPractice(false)" class="w-full bg-[var(--accent-btn)] text-[var(--btn-text)] font-bold py-3.5 rounded-full border-none cursor-pointer transition-transform active:scale-95 text-[17px] flex items-center justify-center gap-2"><i class="fas fa-play text-sm"></i> Begin Session</button>
            </div>`;
    }, 150);
};