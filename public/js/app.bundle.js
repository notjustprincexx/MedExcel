            ...
            if (redeemCard) {
                ...
                if (!alreadyRedeemed && !redeemCard.dataset.wired) {
                    ...
                    const showMsg = (text, ok) => {
                        ...
                        msgEl.style.color = ok ? '#34d399' : '#f87171';
                    ...
                    const submit = async () => {
                        ...
                        if (window._userReferralCode && raw === window._userReferralCode.toUpperCase()) {
                            showMsg("That's your own code 😅", false);
                            return;
                        }

                        if (typeof window.claimReferralCode !== 'function') {
                            showMsg('Hmm, the redeem service isn’t loaded yet. Refresh and try again.', false);
                            return;
                        }
                        ...
                        try {
                            ...
                            // Success — hide the card and update local state so it stays hidden
                            if (typeof window.showToast === 'function') {
                                window.showToast('Referral code applied — bonus rewards unlocked!', 'success');
                            }
                            userData.referredBy = raw;
                            ...
                            redeemCard.style.opacity = '0';
                            redeemCard.style.maxHeight = '0px';
                            redeemCard.style.padding = '0 1.125rem';
                            redeemCard.style.marginBottom = '0';
                            setTimeout(() => { redeemCard.style.display = 'none'; }, 420);
                        } catch (err) {
                            const m = (err && err.message) || 'Could not apply code.';
                            // Map common Cloud Function error shapes to friendlier text
                            let friendly = m;
